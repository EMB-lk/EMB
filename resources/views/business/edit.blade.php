<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Business Profile: {{ $business->name }}
        </h2>
    </x-slot>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $business->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input id="website" type="url" name="website" value="{{ old('website', $business->website) }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @error('website')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="opening_hours" name="opening_hours" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('opening_hours', $business->opening_hours) }}</textarea>
                            @error('opening_hours')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
